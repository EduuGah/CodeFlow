...
describe('divisão do molde', () => {
  it('separa texto e lacunas na ordem em que aparecem', () => {
  ...
  it('a numeração do autor começa em 1, a do código em 0', () => {
  ...
  it('molde sem lacuna nenhuma é um texto só', () => {
    expect(dividirMolde('const x = 1;')).toEqual([{ tipo: 'texto', conteudo: 'const x = 1;' }]);
  });

  it('lacuna repetida conta uma vez só', () => {
...
describe('preenchimento', () => {
  it('coloca a resposta no lugar da lacuna', () => {
    expect(preencher(MOLDE, ['*'])).toBe('function dobro(n) {\n  return n * 2;\n}');
  });

  it('a mesma lacuna repetida recebe a mesma resposta', () => {
    expect(preencher('{{1}} + {{1}}', ['x'])).toBe('x + x');
  });

  it('lacuna vazia vira espaço, não nada', () => {
...
describe('estado do exercício', () => {
  it('lacuna em branco impede a verificação', () => {
...
describe('erros de autoria aparecem no CI, não para o aluno', () => {
  it('numeração com buraco é recusada', () => {
...
describe('a correção aceita respostas diferentes que funcionam', () => {
  ...
  it.each([['*'], ['* 4 /'], ['* 2 / 2 *']])('aceita "%s", porque o resultado está certo', (resposta) => {
    ...
    const r = runProgram(codigo, testes);

    expect(r.error).toBeUndefined();
    expect(r.testResults[0].passed).toBe(true);
  });

  it('recusa uma resposta que não resolve', () => {
    const r = runProgram(preencher(MOLDE, ['+']), testes);
    expect(r.testResults[0].passed).toBe(false);
  });

  it('erro de sintaxe na lacuna vira mensagem, não tela quebrada', () => {
    const r = runProgram(preencher(MOLDE, ['*(']), testes);
